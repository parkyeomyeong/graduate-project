...
class Tracker:
    def __init__(self, detector, frame, axis, grid):
        ...
        # self.awsconnector = AWSconnector
    ...
    def _set_boundary(self, frame, grid):
        boundary0 = frame.shape[(np.abs(self.axis))] // grid
        boundary1 = frame.shape[(np.abs(self.axis))] - boundary0
    ...
    def person_counter(self, frame, person_id):
        ...
        if self.axis == 0:
            ...
            # self.counter_stats = f"people: {self.counter_left - self.counter_right}"
             
            self.counter_in = f"in : {self.counter_left}"
            self.counter_out = f"out : {self.counter_right}"


            #이전과 다르면 통신
            if self.counter_stats != self.pre_counter_stats:
                # self.awsconnector.mqttpublising(self.counter_stats)
                self.pre_counter_stats = self.counter_stats
